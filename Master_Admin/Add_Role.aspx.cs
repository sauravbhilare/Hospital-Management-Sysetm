...
public partial class Master_Admin_Add_Role : System.Web.UI.Page
{
    ...
    public void btnsubmit_OnClick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            cmd.Parameters.AddWithValue("@Role_Name", txtrole.Text);
            cmd.ExecuteNonQuery();
            ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Role Added successfully...!');", true);
        }
        catch (Exception ex)
        {
            ex.ToString();
    ...
    {
        cmd = new SqlCommand();
        cmd.CommandText = "select * from [dbo].[Staff_Role] order by Role_Id desc";
        cmd.Connection = con;
        try
        {
            con.Open();
    ...
    protected void gvProductType_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Delete1")
        {
            ...
            {
                con.Open();
                cmd = new SqlCommand("Delete From [dbo].[Staff_Role] where Role_Id=@Role_Id", con);
                cmd.Parameters.AddWithValue("@Role_Id", id1);
                cmd.ExecuteNonQuery();
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Role Deleted successfully...!');", true);
            }
            catch (Exception ex)
            {
                ex.ToString();
